...
def create_train_dev_set(tokenizer, sentences_pair, is_similar, cn_type, max_sequence_length, validation_split_ratio):
    """
    ...
    cn_type = list(cn_type)
    label_encoder = LabelEncoder()
    integer_encoded = label_encoder.fit_transform(cn_type)
    onehot_encoder = OneHotEncoder(sparse=False)
    onehot_model = onehot_encoder.fit(integer_encoded.reshape(-1, 1))
    joblib.dump(onehot_model, 'onehotencoder_model.pkl')
    cntypes = onehot_encoder.fit_transform(integer_encoded.reshape(-1, 1))
    ...
    cntypes_shuffled = cntypes[shuffle_indices]
    ...
    cntypes_train, cntypes_val = cntypes_shuffled[:-dev_idx], cntypes_shuffled[-dev_idx:]

    return train_data_1, train_data_2, labels_train, leaks_train, val_data_1, val_data_2, labels_val, leaks_val, cntypes_train, cntypes_val


def create_test_data(tokenizer, test_sentences_pair, cn_type, max_sequence_length):
    """
    Create training and validation dataset
    Args:
        tokenizer (keras.preprocessing.text.Tokenizer): keras tokenizer object
    ...
    label_encoder = LabelEncoder()
    integer_encoded = label_encoder.fit_transform(cn_type)
    onehot_encoder = joblib.load('onehotencoder_model.pkl')
    cntypes = onehot_encoder.transform(integer_encoded.reshape(-1, 1))
    ...
    return test_data_1, test_data_2, leaks_test, cntypes
