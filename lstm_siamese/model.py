...
class SiameseBiLSTM:
    ...
    def train_model(self, sentences_pair, is_similar, cn_type, embedding_meta_data, model_save_directory='./'):
        """
        ...
        val_data_x1, val_data_x2, val_labels, leaks_val, cntypes_train, cntypes_val = create_train_dev_set(tokenizer, sentences_pair,
                                                                               is_similar, cn_type, self.max_sequence_length,
        ...
        # cntypes
        cntypes_input = Input(shape=(cntypes_train.shape[1],))
        cntypes_dense = Dense(int(self.number_dense_units/2), activation=self.activation_function)(cntypes_input)

        # Merging two LSTM encodes vectors from sentences to
        # pass it to dense layer applying dropout and batch normalisation
        merged = concatenate([x1, x2, leaks_dense, cntypes_dense])
        ...
        model = Model(inputs=[sequence_1_input, sequence_2_input, leaks_input, cntypes_input], outputs=preds)
        ...
        model.fit([train_data_x1, train_data_x2, leaks_train, cntypes_train], train_labels,
                  validation_data=([val_data_x1, val_data_x2, leaks_val, cntypes_val], val_labels),
